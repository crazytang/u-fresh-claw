...
do_update() {
    ...
    local LATEST_VER=$(curl -s --connect-timeout 10 "https://registry.npmmirror.com/openclaw/2026.4.29" 2>/dev/null | "$NODE_BIN" -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{try{console.log(JSON.parse(d).version)}catch(e){console.log('error')}})" 2>/dev/null)
    ...
    echo -e "  ${CYAN}正在升级...${NC}"
    cd "$CORE_DIR"
    "$NPM_BIN" install openclaw@2026.4.29 --registry=https://registry.npmmirror.com 2>&1
